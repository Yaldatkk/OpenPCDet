...
class PointRCNNHead(RoIHeadTemplate):
    ...
    def save_proposals(self, proposals, output_dir, frame_id):
        """
        Save the proposals to a file, filtering out those with zero points.

        Args:
            proposals (list of torch.Tensor): List of proposals, each proposal is a tensor.
            output_dir (str): Directory where the proposal files will be saved.
            frame_id (int): Frame ID to use for the filename.
        """
        # Create the output directory for the specific frame_id if it does not exist
        frame_output_dir = os.path.join(output_dir, "")
        if not os.path.exists(frame_output_dir):
            os.makedirs(frame_output_dir)
    
        proposal_file = os.path.join(frame_output_dir, f"3D_{frame_id}.txt")
    
        with open(proposal_file, 'w') as f:
            for proposal in proposals:
                # Convert proposal to numpy array
                proposal_np = proposal.cpu().numpy()

                # Filter out proposals with zero points
                # Here, assume `proposal_np[:, 0]` contains the point count
                # Adjust this if the structure is different
                non_zero_proposals = proposal_np[np.sum(proposal_np[:, 3:7], axis=1) > 0]

                # Write filtered proposals to the file
                for proposal_row in non_zero_proposals:
                    line = "Obj " + " ".join(map(lambda x: f"{x:.6f}", proposal_row))
                    f.write(line + "\n")
    ...
    def forward(self, batch_dict):
        ...
        if not self.training:
            ...
            output_dir="/home/saeed/3D_Obj_Det/OpenPCDet/tools/rpn_proposals"
            #print("1" ,frame_id)
            self.save_proposals(batch_box_preds, output_dir,frame_id[0])
            return batch_dict 
